    ...
    */
    ...
    Tree.prototype._addIcon = function (node) {
        if (this._options.showIcon && !(this._options.showImage && node.image)) {
            node.$el.append(
                this._template.icon.node.clone().addClass(node.icon || this._options.nodeIcon)
            );
        }

        if (this._options.showIcon) {
            node.$el.append(
                   $(this._template.titleIcon).append('<img style="width:16px; height:16px;" src="/icon/' + node.titleIcon + '" />')
            );
        }
    }
    ...
    Tree.prototype._template = {
        ...
        icon: {
            ...
            titleIcon: $('<span class="icon"></span>')
        },
